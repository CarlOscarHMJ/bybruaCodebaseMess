function [R,t] = NExT_modified(y,dt,Ts,method)
%
    ...
    case 1
        ...
        for ii=1:Nyy
            for jj=1:Nyy
                y1 = fft(y(ii,:));
                y2 = fft(y(jj,:));
                h0 = ifft(y1.*conj(y2)); % auto-or cross-correlation function
                R(ii,jj,:) = h0(1:M);
                % Corr(x,y) <==> FFT(x)FFT(y)*
                % [c,lag] =xcorr(y(ii,:),y(ii,:));
                % figure;plot(squeeze(R)); hold on; plot(c(N:N+M),'*r')
                %
            end
        end
