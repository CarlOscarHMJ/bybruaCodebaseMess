...
hold on
if any(vertex==1)
    plot([xbounds(1) x1], [max(y(index)) y2], 'Color', [0 0 0 0.3]); % Line to vertex 1

end
if any(vertex==2)
    plot([xbounds(2) x2], [max(y(index)) y2], 'Color', [0 0 0 0.3]); % Line to vertex 2

end
if any(vertex==3)
    plot([xbounds(2) x2], [min(y(index)) y1], 'Color', [0 0 0 0.3]); % Line to vertex 4

end
if any(vertex==4)
    plot([xbounds(1) x1], [min(y(index)) y1], 'Color', [0 0 0 0.3]); % Line to vertex 3
